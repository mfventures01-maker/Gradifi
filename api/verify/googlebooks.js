...
// src/services/verify/server/googleBooksServerHandler.ts
...
async function handleGoogleBooksServerSearch(payload) {
    ...
    return await enqueue({
      ...
      fn: async () => {
        ...
        for (const item of items) {
          ...
          const publishedYear = volumeInfo.publishedDate ? parseInt(volumeInfo.publishedDate.slice(0, 4), 10) : void 0;
          ...
          const match = {
            ...
            provenance: {
              ...
              doi,
              isbn,
              publishedYear: isNaN(publishedYear) ? void 0 : publishedYear,
              publisher: volumeInfo.publisher,
              pageCount: volumeInfo.pageCount,
              categories: Array.isArray(volumeInfo.categories) ? volumeInfo.categories : void 0,
...
// api/verify/googlebooks.ts
async function handler(req, res) {
  if (req.method !== "POST") {
    return res.status(405).json({ error: "Method not allowed" });
  }
  try {
    const result = await handleGoogleBooksServerSearch(req.body);
