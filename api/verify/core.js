...
// src/services/verify/server/coreServerHandler.ts
...
async function handleCoreServerSearch(payload) {
    ...
    return await enqueue({
      ...
      fn: async () => {
        ...
        for (const item of results) {
          const doi = typeof item.doi === "string" ? item.doi.trim() : void 0;
          const title = typeof item.title === "string" ? item.title.trim() : "Untitled CORE Work";
          const authors = Array.isArray(item.authors) ? item.authors.map((a) => typeof a === "string" ? a : a?.name).filter((n) => typeof n === "string" && n.length > 0) : [];
          ...
          const matchType = passage ? passage.text === originalSnippet ? "exact" : "lexical" : "exact";
          const match = {
            ...
            provenance: {
              ...
              authors: authors.length > 0 ? authors : ["Unknown Author"],
              doi,
              publishedYear: typeof item.yearPublished === "number" ? item.yearPublished : void 0,
