...
// src/services/verify/server/persistenceServerHandler.ts
init_dist4();
...
var supabaseUrl = getEnv("VITE_SUPABASE_URL");
var supabaseAnonKey = getEnv("VITE_SUPABASE_ANON_KEY");
...
// src/services/verify/verificationPersistenceService.ts
...
var VerificationValidationError = class extends Error {
  constructor(message) {
    super(message);
    this.name = "VerificationValidationError";
  }
};
var VerificationPersistenceService = class _VerificationPersistenceService {
  /**
   * Generates the deterministic canonical verification ID from a document hash.
   */
  static deriveVerificationId(documentHash) {
   ...
   */
  static validateResultIntegrity(result, providedVerificationId) {
    if (!result) {
   ...
   */
  static buildPersistenceRecord(result, envelope) {
   ...
   */
  async persistVerificationRecord(result, envelope) {
    const recordPayload = _VerificationPersistenceService.buildPersistenceRecord(result, envelope);
    ...
    const getEnvVar = (key) => typeof process !== "undefined" && process.env ? process.env[key] : void 0;
    const serviceRoleKey = getEnvVar("SUPABASE_SERVICE_ROLE_KEY");
    const supabaseUrl2 = getEnvVar("VITE_SUPABASE_URL");
    let client = supabase;
    if (serviceRoleKey && supabaseUrl2) {
      const { createClient: createClient2 } = await Promise.resolve().then(() => (init_dist4(), dist_exports));
      client = createClient2(supabaseUrl2, serviceRoleKey, {
        auth: { persistSession: false, autoRefreshToken: false }
      });
    }
    const { data: existing, error: fetchError } = await client.from("public_verification_records").select("*").eq("verification_id", recordPayload.verification_id).maybeSingle();
    if (fetchError && fetchError.code !== "PGRST116") {
      console.warn("Supabase query returned error (or offline mode):", fetchError.message);
    }
    if (existing) {
      if (existing.document_hash !== recordPayload.document_hash || existing.evidence_hash !== recordPayload.evidence_hash) {
    ...
    const { data: inserted, error: insertError } = await client.from("public_verification_records").insert(recordPayload).select().single();
    if (insertError) {
      if (insertError.code === "23505") {
        const { data: reFetched } = await client.from("public_verification_records").select("*").eq("verification_id", recordPayload.verification_id).single();
        if (reFetched && reFetched.document_hash === recordPayload.document_hash && reFetched.evidence_hash === recordPayload.evidence_hash) {
          return {
            record: reFetched,
            status: "IDEMPOTENT_EXISTS"
          };
...
var verificationPersistenceService = new VerificationPersistenceService();

// src/services/verify/server/persistenceServerHandler.ts
async function handlePersistenceServerRequest(payload) {
  ...
  const serverSupabase = createClient(supabaseUrl2, serviceRoleKey, {
    auth: { persistSession: false, autoRefreshToken: false }
  });
  const recordPayload = VerificationPersistenceService.buildPersistenceRecord(payload.result, payload.envelope);
  const { data: existing, error: fetchError } = await serverSupabase.from("public_verification_records").select("*").eq("verification_id", recordPayload.verification_id).maybeSingle();
  if (fetchError && fetchError.code !== "PGRST116") {
  ...
  const { data: inserted, error: insertError } = await serverSupabase.from("public_verification_records").insert(recordPayload).select().single();
  if (insertError) {
    if (insertError.code === "23505") {
      const { data: reFetched } = await serverSupabase.from("public_verification_records").select("*").eq("verification_id", recordPayload.verification_id).single();
      if (reFetched && reFetched.document_hash === recordPayload.document_hash && reFetched.evidence_hash === recordPayload.evidence_hash) {
        return {
          record: reFetched,
          status: "IDEMPOTENT_EXISTS"
...
// api/verify/persist.ts
async function handler(req, res) {
  if (req.method !== "POST") {
    return res.status(405).json({ error: "Method not allowed" });
  }
  try {
    const result = await handlePersistenceServerRequest(req.body);
