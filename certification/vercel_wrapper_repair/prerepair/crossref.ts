...
// src/services/verify/server/crossrefServerHandler.ts
...
async function handleCrossrefServerSearch(payload) {
    ...
    return await enqueue({
      ...
      fn: async () => {
        ...
        for (const item of items) {
          ...
          const authors = (item.author || []).map((a) => `${a.given || ""} ${a.family || ""}`.trim()).filter((name) => name.length > 0);
          const containerTitle = item["container-title"]?.[0] || "";
          const publishedYear = item.published?.["date-parts"]?.[0]?.[0];
          const isbn = Array.isArray(item.ISBN) ? item.ISBN[0] : void 0;
